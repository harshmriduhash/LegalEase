export async function simplifyTextLocally(
  originalText: string
): Promise<string> {
    ...
    setTimeout(() => {
      try {
        if (originalText.trim().length < 50) {
          resolve(
            `The document appears to be very short (${originalText.length} characters). Here is the content: ${originalText}`
          );
          return;
        }
        ...
        if (paragraphs.length === 0) {
          resolve(
            "The document was processed but no meaningful content was found."
          );
          return;
        }
        ...
        if (
          ...
          entities.monetaryValues.length > 0
        ) {
          simplified += `\n## Important Details\n`;
        ...
        console.log(
          "Legal document simplification complete, length:",
          simplified.length
        );
        resolve(simplified);
      } catch (error) {
        ...
        resolve(
          "An error occurred while trying to simplify the legal document. Please try again."
        );
      }
    }, 1000);
  });
 ...
 */
function identifyDocumentType(text: string): {
  category: string;
  type: string;
} {
...
function extractEntities(text: string): {
  ...
  const partyMatches = text.match(
    /(?:plaintiff|defendant|petitioner|respondent|claimant)(?:\s+|\s*,\s*|\s+is\s+)([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)/g
  );
  if (partyMatches) {
